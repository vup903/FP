...
public:
    ...
    void drawGhost(RenderWindow *window) { window->draw(ghostSpr); }
};

void Ghost::init(float ghostPos_x, float ghostPos_y)
{
    ghostSpr.setScale(sf::Vector2f(0.1, 0.1));
    ghostSpr.setPosition(sf::Vector2f(ghostPos_x, ghostPos_y));
}

Ghost::Ghost(string ghostFile, float ghostPos_x, float ghostPos_y)
{
